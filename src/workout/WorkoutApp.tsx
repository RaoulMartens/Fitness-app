...
import { beginWorkout, cancelPlanDraft, changeWorkout, openWorkspace, workoutDb, type SessionAction } from './db'
import { isRunning, recordId, restRemaining, resultLabel, starterProgram, targets, timerLabel, type Program, type View, type Workout, type WorkoutDraft, type WorkoutSet } from './model'
import { PlanEditor } from './PlanEditor'
import { WorkoutSetForm } from './WorkoutSetForm'
import { useWakeLock } from './useWakeLock'

type Route = { view: View; sessionId?: string }
type Overlay = 'info' | 'demo' | 'resume' | 'interrupt' | 'finish' | 'abort' | null
...
export function WorkoutApp() {
  ...
  const ending = useRef<{ revision: number; draftRevision?: number } | null>(null)
  ...
  const state = useLiveQuery(async () => {
    ...
    try {
      return await workoutDb.transaction('r', workoutDb.workspace, workoutDb.sessions, workoutDb.sets, workoutDb.drafts, async () => {
        ...
        const current = session && targets(session.snapshot)[session.cursor]
        ...
        return { workspace, session, activeSession, sessions, sets, drafts, previous, failure: null }
      })
  ...
  function change(command: SessionAction, destination?: View) {
    if (!session) return
    const draftRevision = state?.failure === null ? state.drafts[0]?.revision : undefined
  ...
  function openSheet(next: Overlay) {
    if ((next === 'finish' || next === 'abort') && session) ending.current = { revision: session.revision, draftRevision: state?.failure === null ? state.drafts[0]?.revision : undefined }
    setSheet(next)
  }
  function start() {
  ...
  const { workspace, sets, drafts, sessions, activeSession } = state
  ...
  const current = allTargets[Math.min(session?.cursor ?? 0, count - 1)]
  const displayed = allTargets[Math.max(0, Math.min((session?.cursor ?? 0) - (session?.rest ? 1 : 0), count - 1))]
  ...
  if (recovery && (!session || session.status !== 'active' || session.rest || recordId(session.id, current.slot.id, current.target.number) !== recovery.id)) return <main className="app loading"><h1>Je training is elders gewijzigd</h1><p role="alert">Deze invoer kon niet worden opgeslagen en is niet als set vastgelegd.</p><p>Jouw invoer: {recovery.weight || 'geen gewicht'} kg · {recovery.reps || 'geen'} herhalingen.</p><p>Neem de waarden zo nodig over voordat je de actuele sessie opent.</p><button className="primary" onClick={reloadInput}>Actuele sessie openen</button></main>

  return <main className="app workout">
    <header className="topbar"><span>Werkend wireframe · M2</span><button className="text-button" disabled={busy} onClick={() => setSheet('info')}>Over deze versie</button></header>
    <div className="test-note">Startschema · apparaatinstructies en video's volgen</div>
    <nav className="workout-nav" aria-label="Sessie"><span>{online ? 'Op dit apparaat' : 'Offline · op dit apparaat'}</span>{showBack && <button className="text-button" disabled={busy} onClick={back}>{training && running ? 'Naar Vandaag · sessie blijft lopen' : safeView === 'plan-edit' ? 'Terug naar Plan' : safeView === 'consequences' ? 'Terug naar je sessie' : 'Terug naar Vandaag'}</button>}</nav>
    <div className="content">
      {error && <div role="alert" className="error"><p>{error}</p><button onClick={() => setError('')}>Sluiten</button></div>}
      {safeView === 'today' && <>
        <h1 tabIndex={-1}>Vandaag</h1>
        {isRunning(activeSession) && activeSession ? <section className="plain-panel"><h2>{activeSession.status === 'paused' ? 'Sessie gepauzeerd' : 'Sessie loopt'}</h2><p>{activeSession.snapshot.name} · {activeSession.cursor} van {targets(activeSession.snapshot).length} sets vastgelegd</p><p className="muted">{activeSession.phase === 'warmup' ? 'Je was bij de warming-up.' : activeSession.cursor === count ? 'Je kunt je sessie afronden.' : `${current.slot.name} · set ${current.target.number}${draft ? ' · concept bewaard' : ''}`}</p><button className="primary" disabled={busy} onClick={() => setSheet('resume')}>Verder trainen</button></section>
          : <section className="plain-panel"><h2>Woensdag · full-body</h2><p>7 oefeningen · ongeveer 60 minuten</p><p className="muted">Je hele lichaam, met extra aandacht voor je bovenlichaam.</p><button className="primary" disabled={busy} onClick={start}>{activeSession ? 'Volgende training starten' : 'Start training'}</button><button className="text-button" onClick={() => go('overview')}>Bekijk de sessie</button></section>}
      ...
      {safeView === 'exercise' && session && <>
        ...
        {session.status === 'paused' ? <section className="plain-panel"><h2>Sessie gepauzeerd</h2><p>Je plek, invoer en resterende rust zijn bewaard.</p><button className="primary" disabled={busy} onClick={() => change('resume', 'exercise')}>Verder trainen</button></section>
          ...
          : session.cursor === count ? <section className="group"><h2>Alle sets vastgelegd</h2><p>Je sessie loopt nog tot je deze afrondt.</p><button className="primary" disabled={busy} onClick={() => openSheet('finish')}>Sessie afronden</button></section>
    ...
    <footer className="footer"><span>{offlineReady || swControlled ? 'App offline beschikbaar' : swError ? 'Offline app nog niet beschikbaar' : import.meta.env.DEV ? 'Ontwikkelversie · offline app via preview' : 'App voorbereiden voor offline gebruik…'}</span><span>Lokale opslag · geen cloudsynchronisatie · demonstraties ontbreken nog</span></footer>
    ...
    {sheet && <Sheet title={{ info: 'Over deze versie', demo: 'Oefenuitleg', resume: 'Verder trainen', interrupt: 'Pauzeren of stoppen', finish: 'Sessie afronden?', abort: 'Sessie afbreken?' }[sheet]} onClose={() => { if (!busy) setSheet(null) }}>
      {error && <p className="error" role="alert">{error}</p>}
      {sheet === 'info' && <><p>Low fidelity, echte opslag. Je sessie, setinvoer en rusttijd blijven op dit apparaat bewaard.</p><p>Apparaatinstructies en demonstraties zijn nog niet beschikbaar. Laat de instelling en uitvoering bij de eerste training ter plaatse uitleggen.</p><a href={`${import.meta.env.BASE_URL}?m1=1`}>M1-testregistraties bekijken</a><a href={`${import.meta.env.BASE_URL}m2-voorstel/`}>Ontwerpvoorstel bekijken</a><p className="note">Dit is nog geen cloudsynchronisatie. Wissen van browsergegevens verwijdert lokale registraties.</p></>}
...
function Results({ session, sets }: { session: Workout; sets: WorkoutSet[] }) {
  return <ul className="exercise-list">{session.snapshot.slots.map(slot => { const records = sets.filter(item => item.slotId === slot.id).sort((a, b) => a.number - b.number); return <li key={slot.id}><div><strong>{slot.name}</strong>{records.length ? records.map(record => <p className="note" key={record.id}>Set {record.number}: {formatWeight(record.weight)} kg × {record.reps}</p>) : <p className="note">Niet uitgevoerd</p>}</div><span>{records.length}/{slot.sets.length}</span></li> })}</ul>
}
